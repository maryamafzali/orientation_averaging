...
ph = acos(0.4/sin(te));
fdir2 = [sin(te+pi/2)*cos(ph) sin(te+pi/2)*sin(ph) cos(te+pi/2)];
...
for i = 1:s1 % b-values
    ...
    for l = 1:s4 % sigma_g
        for n = 1:s5 % kappa = \infty, 1, 9
            if n == 1
                S(:,i,l,n) = 0.5*exp(-b*D_per-b*(D_par - D_per).*(bvec*fdir1').^2) + ...
                    0.5*exp(-b*D_per-b*(D_par - D_per).*(bvec*fdir2').^2);
            elseif n == 2 || n == 3
                S(:,i,l,n) = 0.5*slow_exchange_b_tensor_SynthMeasWatsonHinderedDiffusion_PGSE([D_par D_per kappa(n-1)], ...
                    bvec, b, fdir1',1) + 0.5*slow_exchange_b_tensor_SynthMeasWatsonHinderedDiffusion_PGSE([D_par D_per kappa(n-1)], ...
                    bvec, b, fdir2',1);
            end
        end
    end
