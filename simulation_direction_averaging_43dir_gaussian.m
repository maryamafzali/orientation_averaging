...
% load X_152_Qdistr_03756__1.00_2.00_2.00_1.00_0.00_J0_S1_4.00_1.50_0.21.mat % non_shelled
% load X_152_Qdistr_03762__1.00_2.00_2.00_1.00_0.00_J0_S1_4.00_1.50_0.21.mat % shelled
% bvec = rivec1(:,1:2:304);

% for i = 1:length(bvec)
%     norm_bvec(i) = (norm(bvec(:,i))/0.9)^2*12000;
% end
...
Siso_shell = zeros(p,3,2,3, s4, 100);
for p = 1:100 % noise realization
    for l = 1:s4 % sigma_g
        for j = 1:3 % kappa
            for i = 1:2 % Nmax
        ...
        for j = 1:3 % kappa
            for i = 1:2 % Nmax
                for r = 1:3 % method
                    ...
                    [kappa_iso_no_dir(1:Nmax(i)/2+1,j,i,r,l,p),E, u0_no_dir(j,i,r,l,p)] = ...
...
bvals2 = 0:1000/4:20000;
Siso_new_no_dir = zeros(length(bvals2),3,2,3, s4, 100);
mag_q = sqrt(bvals2/td)/(2*pi);
        ...
        for j = 1:3 % kappa
            for i = 1:2 % Nmax
                for r = 1:3 % method
                    ...
                    for k = 1:Nmax(i)/2 + 1
                        E(:,k) = mapmri_Xi_00(k,u0_no_dir(j,i,r,l,p),mag_q);
                    end
                    
                    Siso_new_no_dir(:,j,i,r,l,p) = E(:,1:Nmax(i)/2+1)*...
                        squeeze(kappa_iso_no_dir(1:1:Nmax(i)/2+1,j,i,r,l,p));
                end
            end
        end
    end
