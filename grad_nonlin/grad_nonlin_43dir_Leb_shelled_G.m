...
rng(1)
S = zeros(ndir,s1,s4,s5);
for i = 1:s1 % b-values
%     b = (i-1)*1.5*ones(length(bvec),1);
%     b = (i-1)*1.5*(ones(length(bvec),1)+(-1)^round(rand)*rand(length(bvec),1)/10);
    b = (i-1)*1.5*(ones(length(bvec),1)+((-1).^round(rand(length(bvec),1))).*rand(length(bvec),1)/10);
    for l = 1:s4 % sigma_g
        for n = 1:s5 % kappa = \infty, 1, 9
            if n == 1
                S(:,i,l,n) = exp(-b*D_per-b*(D_par - D_per).*(bvec*fdir').^2);
            elseif n == 2 || n == 3
...
rng(1)
b = [0*(ones(length(bvec),1)+(-1)^round(rand)*rand(length(bvec),1)/10); ...
    1.5*(ones(length(bvec),1)+(-1)^round(rand)*rand(length(bvec),1)/10); ...
    3*(ones(length(bvec),1)+(-1)^round(rand)*rand(length(bvec),1)/10); ...
    4.5*(ones(length(bvec),1)+(-1)^round(rand)*rand(length(bvec),1)/10); ...
    6*(ones(length(bvec),1)+(-1)^round(rand)*rand(length(bvec),1)/10);...
    7.5*(ones(length(bvec),1)+(-1)^round(rand)*rand(length(bvec),1)/10); ...
    9*(ones(length(bvec),1)+(-1)^round(rand)*rand(length(bvec),1)/10); ...
    10.5*(ones(length(bvec),1)+(-1)^round(rand)*rand(length(bvec),1)/10); ...
    12*(ones(length(bvec),1)+(-1)^round(rand)*rand(length(bvec),1)/10)];
...
ia = 1:ndir:length(b);
pl = 9;
GT = sqrt(pi)/2*exp(-D_per*C/1000).*erf(sqrt((D_par - D_per)*C/1000))./sqrt((D_par - D_per)*C/1000);
Siso_shell = zeros(pl,3,2,3, s4, 100);
for p = 1:100 % noise realization
    for l = 1:s4 % sigma_g
        for j = 1:3 % kappa
            for i = 1:2 % Nmax
                for r = 1:3 % method
                    ...
                    Siso_sort(:,1) = S_iso(order,j,i,r,l,p);
                    for k = 1:pl-1
                        Siso_shell(k,j,i,r,l,p) = mean(Siso_sort(ia(k):ia(k+1)-1));
                    end
                    Siso_shell(pl,j,i,r,l,p) = mean(Siso_sort(ia(pl):end));
                end
            end
        end
    end
        ...
        for j = 1:3 % kappa
            for i = 1:2 % Nmax
                for r = 1:3 % method
                    ...
                    S1 = squeeze(mean_Hans(:,l,j,p));
                    
                    [kappa_iso_no_dir,E, u0_no_dir(j,i,r,l,p)] = ...
                        mapmri_iso_no_dir(S1,bvals2',big_delta,little_delta,Nmax(i),method);
                    
                    S_iso_no_dir(:,j,i,r,l,p) = E*squeeze(kappa_iso_no_dir(1:Nmax(i)/2+1,j,i,r,l,p));
